...
/* Initialize 4-D pheromone array */
    double *phMatrix;
    if(NULL == (phMatrix = malloc(sizeof *phMatrix * G->n_depots*G->n_nodes*G->n_nodes))){
    ...
    int **K;
    if(NULL == (K = malloc(sizeof *K * (G->n_nodes)))){
        printf("Error mallocing clusters K\n");
        exit(1);
    }
    int rows = (int)ceil((double)G->n_customers/n_size);
    int counter = 0;
    for(int i = 0; i < (G->n_nodes); i++){
        if(NULL == (K[i] = calloc(rows * n_size, sizeof *K[i]))){
            perror("Error callocing K[ivt][i] clusters..\n"); 
            exit(1);
        }
        createClusters(K[i], G, i, n_size, n_prim, n_sect);
        counter++;
    }
    ...
    R.total_distance = HUGE_VAL;             //Initialize final solution distance
    ...
    /********** Main loop **********/
    while(stop_cond < 10000 && iter < 10000000 && term_condition > (0.1 - epsilon) && loop_time < 30)
    {
        ...
        for(int i = 0; i < n_ants; i++){
            ...
            antSolution(G, K, phMatrix, &Ra_local, n_size, n_prim, a, b);
        ...
        if (iter % n_freq == 0) {
            R_best.total_distance = local_opt_full(&R_best, G);
        }
    ...
    printf("\nTotal Makespan: %lf", R.total_distance);
