...
double local_opt_full(vt_solution *R, SON *G){
    ...
    while(flag){
        for (int idep = 0; idep < G->n_depots; idep++) {
            double ms1 = k_optimization2(&R->a_depots[idep], G, 1);
            double ms2 = k_optimization2(&R->a_depots[idep], G, 2);
        }
        //R->total_distance = get_makespan_VT(G, &R);
        double og = R->total_distance;
        double ms1 = mutual_k_optimization(R, G, 1, 1);
        double ms2 = mutual_k_optimization(R, G, 1, 2);

        double ms3 = mutual_k_optimization(R, G, 2, 1);
        double ms4 = mutual_k_optimization(R, G, 2, 2);
...
double k_optimization2(route *r, SON *G, int n_max){
    ...
    while(list_position < list_length-n_max){                           //For every node(s) in depotroute
        ...
        while(p && p->next){                    //While depotroute has nodes, place successive nodes after them
            ...
            int check = check_route_feasibility(depotroute, G);
            if(check == 0){
        ...
        while(sec < G->n_depots){
            ...
            while(l1_position < length_l1-n_max){                                   //For every node(s) in l1
                ...
                while(p && p->next){
                    ...
                    int check = check_route_feasibility(l2, G);
                    if(check == 0){
