...
int selectCluster(int idepot, int ilast, int *v_free, int *K, double *phMatrix,
    ...
    for(icluster = 0; icluster < n_prim; icluster++){
        find_free_in_icluster(&v_cand, v_free, K, icluster, n_size);
        if(!v_cand){
            ...
            t[icluster] = calculate_pheromone_sum(idepot, ilast, v_cand, phMatrix, G->n_nodes) / numOfCand;               
            deleteList(&v_cand);
    ...
    if(h_sum == 0.0){
        for(int icluster = n_prim; icluster < (int)ceil((double)G->n_customers / n_size); icluster++){
            if(search_icluster(K, icluster, v_free, n_size)){
                return icluster;
            }
        }
    }else{
