...
void createClusters(int *K, SON *G, int vi, int n_size, int n_prim, int n_sect){
    ...
    polar *polarArray;
    if(NULL == (polarArray = malloc(sizeof *polarArray * G->n_customers))){
        printf("Error mallocing polarArray in createClusters.c\n");
        exit(1);
    }
    find_polar_coordinates(G, vi, polarArray);

    sector *sec;
    if(NULL == (sec = malloc(sizeof *sec * n_sect))){
        printf("Error mallocing sec in createClusters.c\n");
        exit(1);
    }
    ...
    for (int j = 0; j < n_sect; j++){
        ...
        closest = find_closest_free_vertex_in_sector(polarArray, sec[j].vertexList);
        if(closest != -1){
            if(closest > G->n_customers){ printf("Error in createCluster\nclosest = %d\n", closest); exit(1); }
...
/* Continue filling cluster 1 and then the rest */ //(n_sect<n_size)
    while(not_empty(v_free, G->n_customers)){
        closest = find_closest_free_vertex(v_free, polarArray, G->n_customers);
        if(closest != -1){
            if(closest > G->n_customers){ printf("Error in createCluster2\n"); exit(1); }
    ...
    for(int i = 0; i < n_sect; i++) {
        if(sec[i].vertexList != NULL){
            deleteList(&sec[i].vertexList);
        }
    }  
    if(!polarArray){
        printf("!polarArray\n");
