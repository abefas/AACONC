...
int selectDepot(vt_solution *Ra, int *v_free, SON *G, int **K, double *phMatrix, int n_size, int n_prim){
    ...
    while(p_sum == 0.0){
        for(int idepot = 0; idepot < G->n_depots; idepot++){
            ...
            for(int icluster = 0; icluster < N; icluster++)
                find_free_in_icluster(&v_cand, v_free, K[ilast], icluster, n_size);

            if(v_cand){
                prob_D = calculate_pheromone_sum(idepot, ilast, v_cand, phMatrix, G->n_nodes);
                deleteList(&v_cand);
            }
            if(prob_D > 0.0){
